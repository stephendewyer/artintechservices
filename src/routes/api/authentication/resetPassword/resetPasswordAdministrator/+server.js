...
export async function POST({request}) {
    ...
    // get the user from administrators table in database

    const checkEmailQuery = `SELECT email FROM administrators WHERE email = '${email}';`;
    /**
     * @type {string | any[]}
     */
    ...
    if (administratorAccount.length === 0) {
        return new Response(JSON.stringify({error: `no administrator account with ${email} as email exists!`}), {status: 422});
    };
    ...
    const tokenUpdateStatement = `UPDATE administrators
    SET 
        reset_token = "${token}", 
    ...
    const msg = {
        ...
        html: `
            ...
            <p>Click <a href="${DOMAIN}/update-administrator-password?token=${token}&email=${email}" >this link to set a new password.</a></p>
            <p>Kind regards,</p>
